...
Monitor.prototype._init = function (options) {
  ...
  var pm2Daemon = options.pm2Conf.DAEMON_PUB_PORT;
  if (!pm2Daemon || !fs.existsSync(pm2Daemon)) {
    throw new Error(fbMsg + 'Unfortunately ' + (pm2Daemon || 'DAEMON_PUB_PORT') + ' can not found, please makesure that your pm2 is running and the home path is correct.');
  }
  // Bind to context.
